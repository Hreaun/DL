...
class KNN:
    ...
    def compute_distances_no_loops(self, X):
        ...
        """
        num_test = X.shape[0]
        rX = np.reshape(X, (num_test, 1, X.shape[1]))
        dists = np.sum(np.abs(rX - self.train_X), axis=2)
        return dists
    ...
    def predict_labels_binary(self, dists):
        ...
        for i in range(num_test):
            # сортируем точки по расстоянию до них
            inds = np.argsort(dists[i])

            # берем ближайшие k точек
            labels = [self.train_y[index] for index in inds][:self.k]

            # создаем counter для нахождения most_common меток
            labels_counter = collections.Counter(labels)
            pred[i] = labels_counter.most_common(self.k)[0][0]
        return pred
